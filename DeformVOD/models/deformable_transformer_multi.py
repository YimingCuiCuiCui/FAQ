...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, class_embed = None ):
        ...
        # new_spatial_shapes = spatial_shapes[-1,:].unsqueeze(0)
        # new_valid_ratios = valid_ratios[:,-1,:].unsqueeze(1)

        memory_list = torch.chunk(memory, self.num_ref_frames+1,  dim=0)
        # ref_spatial_shapes = new_spatial_shapes.expand(self.num_ref_frames, 2).contiguous()
        ...
        # new_valid_ratios = new_valid_ratios[0:1].expand(1, self.num_ref_frames, 2)

        # reference_points = self.get_reference_points(new_spatial_shapes, new_valid_ratios, device=cur_memory.device)
